...
return order;}
function buildGrid(){gridEl.innerHTML="";state.tiles=[];for(var i=0;i<TILE_COUNT;i+=1){var tile=document.createElement("button");tile.type="button";tile.className="tile-8b5894cf";tile.disabled=true;tile.setAttribute("aria-label","Tile "+(i+1));tile.innerHTML='<span class="tile-inner-8b5894cf">'+'<span class="tile-face-8b5894cf tile-front-8b5894cf"></span>'+'<span class="tile-face-8b5894cf tile-back-8b5894cf"></span>'+"</span>";tile.addEventListener("click",makeTileHandler(i));gridEl.appendChild(tile);state.tiles.push(tile);}}
function makeTileHandler(index){return function(){onTilePick(index);};}
function revealTile(index,isMine,dim){var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-8b5894cf")){return;}
var back=tile.querySelector(".tile-back-8b5894cf");if(back){back.innerHTML=isMine?BOMB_SVG:GEM_SVG;}
tile.classList.add("revealed-8b5894cf");if(isMine){tile.classList.add("boom-8b5894cf");}
if(dim){tile.classList.add("dimmed-8b5894cf");}
tile.disabled=true;}
function revealRemaining(dimAll){for(var i=0;i<TILE_COUNT;i+=1){if(!state.tiles[i].classList.contains("revealed-8b5894cf")){revealTile(i,state.mineMap[i]===true,dimAll);}}}
function setTilesEnabled(enabled){for(var i=0;i<TILE_COUNT;i+=1){var tile=state.tiles[i];tile.disabled=!enabled||tile.classList.contains("revealed-8b5894cf");}}
...
var tile=state.tiles[index];if(!tile||tile.classList.contains("revealed-8b5894cf")){return;}
...
function bust(){var lost=state.bet;revealRemaining(true);setStatus("Boom! The mine took your "+formatMoney(lost)+" bet.","lose");if(panelEl){panelEl.classList.remove("jolt-8b5894cf");void panelEl.offsetWidth;panelEl.classList.add("jolt-8b5894cf");}
endRound();}
